...
The sandbox intentionally does not reproduce every production cost driver. It uses a Single-AZ RDS instance, keeps the Auto Scaling range small, and uses one Redis node when the sandbox credit balance supports it. NAT Gateway is not included because it is not required by the project.
...
4. Avoid resources that are not required by the project or the sandbox validation.
